...
import { kbAPI } from '@/api/generalAPI'
...
export default {
  ...
  methods: {
    ...
    getList(val) {
      ...
      kbAPI.getData(this.apiName, this.listQuery).then(res => {
    ...
    getDetail(row) {
      ...
      this.isEdit = true
      kbAPI.getForm(this.apiName, row.id).then(res => {
    ...
    getForm(id) {
      this.formTitle = this.$t('dialog.edit') + this.dialogTitle // '编辑'
      this.isEdit = true
      kbAPI.getForm(this.apiName, id).then(res => {
        this.form = res
        this.dialogFormVisible = true
      })
    },
    ...
    /* 删除，默认以id字段提交 */
    handleDelete() {
      this.delete('id', kbAPI.dataPost) // 可在页面自定义
    },
